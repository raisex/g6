...
class HodlContractTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("setup contract account with FLG") {
    val tx =
  ...
  test("setup caller account with FLG") {
    val tx =
      TransferTransactionV2
        .selfSigned(
          version = 2,
  ...
  test("set contract to contract account") {
    val scriptText =
      """
        ...
        |   if (isDefined(pmt.asset)) then throw("can hodl FLG only at the moment")
        |   else {
  ...
  test("caller deposits FLG") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          p = Some(ContractInvocationTransaction.Payment(1.5.FLG,None)),
          timestamp = System.currentTimeMillis(),
          fee = 1.FLG
        )
        .explicitGet()
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 1.5.FLG)
    val balanceAfter = sender.accountBalances(contract.address)._1

    (balanceAfter - balanceBefore) shouldBe 1.5.FLG
  }
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.51.FLG))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.FLG
        )
  ...
  test("caller can withdraw less than he owns") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.49.FLG))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.FLG
        )
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 0.01.FLG)
    (balanceAfter - balanceBefore) shouldBe -1.49.FLG
  }
}
