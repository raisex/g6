...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade FLG-USD" - {
    ...
    "place usd-waves order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("FLG") shouldBe sellOrderAmount + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("FLG") shouldBe bobWavesBalanceBefore - (sellOrderAmount + matcherFee)
    ...
    "get opened trading markets. USD price-asset " in {
      ...
      markets.amountAssetName shouldBe "FLG"
    ...
    "check reserved balance" in {
      ...
      matcherNode.reservedBalance(bobAcc)("FLG") shouldBe expectedBobReservedBalance
    ...
    "check waves-usd tradable balance" in {
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("FLG") shouldBe expectedBobTradableBalance
      matcherNode.tradableBalance(aliceAcc, wavesUsdPair)("FLG") shouldBe aliceNode.accountBalances(aliceAcc.address)._1
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("FLG") shouldBe bobNode.accountBalances(bobAcc.address)._1
    }
  }

  "Alice and Bob trade FLG-USD check CELLING" - {
    val price2           = 289
    ...
    "place usd-waves order" in {
      // Alice wants to sell USD for Waves
      val bobWavesBalanceBefore = matcherNode.accountBalances(bobAcc.address)._1
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("FLG")
      val bobOrder1   = matcherNode.prepareOrder(bobAcc, wavesUsdPair, OrderType.SELL, sellOrderAmount2, price2)
      val bobOrder1Id = matcherNode.placeOrder(bobOrder1).message.id
      matcherNode.waitOrderStatus(wavesUsdPair, bobOrder1Id, "Accepted", 1.minute)

      matcherNode.reservedBalance(bobAcc)("FLG") shouldBe correctedSellAmount2 + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("FLG") shouldBe bobWavesBalanceBefore - (correctedSellAmount2 + matcherFee)
  ...
  "Alice and Bob trade WCT-USD 1" - {
    ...
    "place wct-usd order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("FLG") shouldBe expectedReservedWaves
  ...
  "Alice and Bob trade WCT-FLG on not enough fee when place order" - {
    ...
    "bob lease all  exact half matcher fee" in {
      ...
      matcherNode.tradableBalance(bobAcc, wctWavesPair)("FLG") shouldBe matcherFee / 2 + receiveAmount(SELL, wctWavesSellAmount, wctWavesPrice) - matcherFee
