...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
      | FLG.fees.transfer.$assetId = 100000
      | FLG.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("FLG.miner.enable=no").withFallback(minerConfig)
