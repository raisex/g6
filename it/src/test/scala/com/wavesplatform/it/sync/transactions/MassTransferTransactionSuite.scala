...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.FLG balance is decreased by fee.") {
  ...
  test("FLG mass transfer changes FLG balances") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough FLG") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative FLG balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.FLG), Transfer(secondAddress, 2.FLG), Transfer(thirdAddress, 3.FLG))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.FLG)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.FLG)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.buildWithCurrentChainId(alias).explicitGet().stringRepr, 2.FLG)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
