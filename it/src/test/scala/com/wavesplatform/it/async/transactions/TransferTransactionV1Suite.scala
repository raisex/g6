...
class TransferTransactionV1Suite extends BaseTransactionSuite with TransferSending with CancelAfterFailure {
  ...
  private val transferAmount       = 5.FLG
  private val leasingAmount        = 5.FLG
  private val leasingFee           = 0.003.FLG
  private val transferFee          = 0.002.FLG
  private val issueFee             = 5.FLG

  test("asset transfer changes sender's and recipient's asset balance; issuer's.FLG balance is decreased by fee") {
    val f = for {
  ...
  test("FLG transfer changes FLG balances and eff.b.") {
    val f = for {
      ((firstBalance, firstEffBalance), (secondBalance, secondEffBalance)) <- notMiner
        .accountBalances(firstAddress)
        .zip(notMiner.accountBalances(secondAddress))
  ...
  test("invalid signed FLG transfer should not be in UTX or blockchain") {
    def invalidByTsTx(ts: Long) =
      TransferTransactionV1
        .selfSigned(None, sender.privateKey, AddressOrAlias.fromString(sender.address).right.get, 1, ts, None, 1.FLG, Array.emptyByteArray)
        .right
        .get
  ...
  test("can not make transfer without having enough of FLG") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)
      ...
      transferFailureAssertion <- assertBadRequest(sender.transfer(secondAddress, firstAddress, secondBalance + 1.FLG, transferFee))
  ...
  test("can not make transfer without having enough of your own FLG") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)
