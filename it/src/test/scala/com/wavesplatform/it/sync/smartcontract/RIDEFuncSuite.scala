...
class RIDEFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(acc0.address, newAddress, 10.FLG, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransactionV2
          .selfSigned(2, None, pkNewAddress, pkNewAddress, 1.FLG, System.currentTimeMillis(), None, smartMinFee, Array())
          .explicitGet()
          .json()),
