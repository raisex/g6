          ...
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative FLG balance to (at least) temporary negative state, current balance equals $oldWavesBalance, " +
    ...
    if (height >= Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) {
      for {
        ...
        _ <- Either.cond(
          ...
          (),
          GenericError(
            s"Fee in ${feeAssetId.fold("FLG")(_.toString)} for ${tx.builder.classTag} does not exceed minimal value of $minWaves FLG: ${tx.assetFee._2}")
        )
      } yield ()
    } else {
