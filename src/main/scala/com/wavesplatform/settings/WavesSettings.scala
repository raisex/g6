...
object WavesSettings {
  ...
  def fromConfig(config: Config): WavesSettings = {
    ...
    val walletSettings          = config.as[WalletSettings]("FLG.wallet")
    ...
    val utxSettings             = config.as[UtxSettings]("FLG.utx")
    val featuresSettings        = config.as[FeaturesSettings]("FLG.features")
